...
카피를 반환하되, 16종의 세부 문구는 최종 콘텐츠 검수 대상으로 남아 있다. 사용 방법도 확정된
네 슬롯의 조합 카피를 반환한다. 주의사항과 충전 방법은 문구 조합 규칙이 확정될 때까지 목업값을
사용한다.
...
화면에 표시할 문자열 4개를 반환한다. 문구 조합 규칙은 아직 확정되지 않아 현재 목업값을
사용한다.
