...
충전 방법은 55자 이하의 메인 문장과 다음 순서의 키워드 3개를 반환한다.

1. 평소 회복: 평소 휴일의 첫 행동인 `step1.q07`
2. 일정 변경 시 환기: 갑자기 약속이 취소됐을 때의 행동인 `step1.q08`
3. 감정 회복: 기분이 좋지 않을 때 원하는 반응을 자기 행동으로 해석한 `step1.q12`

메인 문장은 `step1.q07`의 평소 회복 행동과 `step1.q11`의 다시 기운이 올라오는 계기를
연결한다. MBTI는 사용하지 않는다. 선택지 문장을 그대로 반복하지 않고 네 답변을 실제 생활에서
하는 충전 행동으로 바꿔 반환한다.

| 필드 | 타입 | 설명 |
|---|---|---|
| `description` | string | `step1.q07 × step1.q11`로 정해지는 55자 이하의 충전 패턴 문장 |
...
희귀도와 상단 태그는 아직 화면 연동용 목업값을 사용한다.
...
```json
{
  ...
  "charging": {
    "description": "알람 없이 늦잠을 자며 먼저 푹 쉬고, 새로운 구경거리가 생기면 다시 기운이 올라와요.",
    "activities": [
