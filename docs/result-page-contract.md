...
주의사항도 각각 확정된 네 슬롯의 조합 카피를 반환한다. 충전 설명과 활동은 `step1.q07 ×
step1.q08`의 24개 조합으로 결정하며, 충전 점수만 계산 규칙이 확정될 때까지 목업값을 사용한다.
...
충전 방법은 다음 순서의 활동 3개와 조합 설명을 반환한다.

1. 기본 충전법: 평소 휴일의 첫 행동인 `step1.q07`
2. 비상 충전법: 갑자기 약속이 취소됐을 때의 행동인 `step1.q08`
3. 고속 충전법: `step1.q07 × step1.q08`의 24개 조합

MBTI는 사용하지 않는다. 사용자가 실제로 고른 두 행동을 선택지 그대로 반복하지 않고 회복
패턴과 실천 가능한 활동으로 바꿔 반환한다.

| 필드 | 타입 | 설명 |
|---|---|---|
| `score` | integer | 충전 점수. 현재 목업값 `90` |
| `description` | string | `step1.q07 × step1.q08` 조합으로 정해지는 충전 패턴 설명 |
...
희귀도·상단 태그와 충전 점수는 아직 화면 연동용 목업값을 사용한다.
...
```json
{
  ...
  "charging": {
    "score": 90,
    "description": "일정이 비면 아쉬워하기보다 잠부터 보충해요. 누구의 속도에도 맞추지 않고 충분히 자야 배터리가 제대로 차는 타입이에요.",
    "activities": [
      {"type": "sleep_until_noon", "label": "늦잠으로 밀린 잠 갚기"},
      {"type": "go_to_bed", "label": "비어버린 약속만큼 푹 쉬기"},
      {"type": "sleep_until_noon_go_to_bed", "label": "휴대폰을 멀리 두고 알람 없이 푹 자기"}
    ]
  }
}
```
